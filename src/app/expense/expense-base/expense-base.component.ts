...
import {ExpenseTypeDialogComponent} from '../../expense-type/dialogs/expense-type-dialog/expense-type-dialog.component';
import {VendorDialogComponent} from '../../vendor/dialogs/vendor-dialog/vendor-dialog.component';
...
export class ExpenseBaseComponent implements OnInit, OnDestroy {
    ...
    constructor(protected injector: Injector) {
        ...
        this._subscriptions.push(this.expenseTypes$.subscribe(types => this.expenseTypes = types));
        this._subscriptions.push(this.vendors$.subscribe(vendors => this.vendors = vendors));
        this._subscriptions.push(AppService.stateRefreshed.subscribe(refreshed => {
            if (!refreshed) { return; }
            setTimeout(() => this.setUpFormValues(), 50)
        }));
    }
    ...
    addExpenseType() {
        ...
        d.componentInstance.onSaved.subscribe(item => setTimeout(() => {
            d.close();
        }));
    }
